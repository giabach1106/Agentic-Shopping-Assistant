...
import { Search, Filter, SlidersHorizontal, ArrowRight, CheckCircle2, Loader2, AlertCircle, ChevronDown, ExternalLink, Star, Clock } from 'lucide-react';
            ...
            <div className="grid grid-cols-1 gap-6">
              {filteredResults.length > 0 ? filteredResults.map((product, index) => (
                <div key={product.id} className="bg-white rounded-xl border border-zinc-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col md:flex-row">
                  <div className="w-full md:w-64 h-48 md:h-auto bg-zinc-100 relative shrink-0">
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                    ...
                    <div className="mt-6 flex items-center gap-3">
                      <Link href={`/product/${product.id}`} className="flex-1 text-center px-4 py-2.5 bg-zinc-100 hover:bg-zinc-200 text-zinc-900 rounded-xl text-sm font-medium transition-colors">
                        View Full Analysis
                      </Link>
                      <button onClick={() => window.open('https://amazon.com', '_blank')} className="flex-1 px-4 py-2.5 bg-orange-600 hover:bg-orange-700 text-white rounded-xl text-sm font-medium transition-colors flex items-center justify-center gap-2">
