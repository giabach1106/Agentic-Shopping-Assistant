...
from app.services.evidence_precision import normalize_collection_evidence
...
QUERY_BANKS = {
    ...
    "desk": [
        ...
        "standing desk",
        "computer desk",
        "desk under 55 inches",
    ],
}

QUERY_MODIFIERS = {
    "supplement": ["powder", "capsules", "vanilla", "chocolate", "unflavored", "third party tested", "low lactose"],
    "chair": ["mesh", "lumbar", "adjustable arms", "home office"],
    "desk": ["with storage", "small space", "standing", "home office"],
}
...
_CHAIR_KEYWORDS = ("chair", "ergonomic", "lumbar", "mesh", "office chair", "desk chair")
_DESK_KEYWORDS = ("desk", "study desk", "writing desk", "standing desk", "office desk")
_TARGET_COMMERCE_SOURCES = {
    "supplement": {"amazon", "ebay", "nutritionfaktory", "dps"},
    "chair": {"amazon", "ebay", "walmart"},
    "desk": {"amazon", "ebay", "walmart"},
}
...
def _is_relevant_domain_text(text: str, domain: str) -> bool:
    lower = text.lower()
    if domain == "chair":
        return any(keyword in lower for keyword in _CHAIR_KEYWORDS)
    if domain == "desk":
        return any(keyword in lower for keyword in _DESK_KEYWORDS)
    return _is_relevant_supplement_text(text)
...
def _build_records(
    ...
    for item in products:
        ...
        if not source or not url.startswith("http") or not title:
            ...
            continue
        if source not in _TARGET_COMMERCE_SOURCES.get(domain, _TARGET_COMMERCE_SOURCES["supplement"]):
        ...
        if _is_search_listing_url(url):
            ...
            continue
        if not _is_relevant_domain_text(title, domain):
...
async def warmup_catalog(target_records: int, *, domain: str = "supplement") -> dict[str, Any]:
    settings = Settings.from_env()
    collector = LiveRealtimeCollector(settings)
    ...
    try:
        ...
        for base in QUERY_BANKS.get(domain, QUERY_BANKS["supplement"]):
            expanded_queries.append(base)
            for modifier in QUERY_MODIFIERS.get(domain, QUERY_MODIFIERS["supplement"]):
        ...
        for query in expanded_queries:
            ...
            public_payload = normalize_collection_evidence(
                collected.to_public_dict(),
                constraints={"category": query, "mustHave": [domain]},
            )
            records = _build_records(public_payload, domain=domain, rejection_counts=rejection_counts)
            for record in records:
                deduped[str(record["url"])] = record
                if len(deduped) >= target_records:
                    break
            for record in public_payload.get("evidenceRecords", []):
    ...
    await store.upsert_evidence_records(list(evidence_records.values()))
    metrics = await store.catalog_metrics()
    metrics["domain"] = domain
    metrics["targetRecords"] = target_records
    metrics["seededRecords"] = len(deduped)
    metrics["seededEvidenceRecords"] = len(evidence_records)
    ...
    metrics["sourcesTargeted"] = sorted(_TARGET_COMMERCE_SOURCES.get(domain, _TARGET_COMMERCE_SOURCES["supplement"]))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Warm up a domain-specific offer and evidence corpus.",
    )
    parser.add_argument(
        "--target",
        type=int,
    ...
    parser.add_argument(
        "--domain",
        choices=("supplement", "chair", "desk"),
        default="supplement",
        help="Domain to warm.",
    )
    args = parser.parse_args()

    metrics = asyncio.run(warmup_catalog(max(10, args.target), domain=args.domain))
    ...
    print(f"Seeded evidence records: {metrics['seededEvidenceRecords']}")
